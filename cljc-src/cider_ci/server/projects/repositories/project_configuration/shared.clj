...
(defn- get-commit-ids [git-refs]
  (let [query (-> (sql/select-distinct :commits.id)
                  (sql/from :commits)
                  (sql/join :branches_commits
                            [:=
                             :commits.id
...
(defn resolve-tree-id-for-commit-ids [commit-ids]
  (let [query (-> (sql/select :commits.tree_id :commits.committer_date)
                  (sql/from :commits)
                  (sql/where [:in :commits.id commit-ids])
                  (sql/join :branches_commits [:=
                                                    :commits.id
                                                    :branches_commits.commit_id])
                  (sql/limit 1)
...
(defn- get-commit-refs-for-submodule [commit-ids path]
  (let [query (-> (sql/select :submodules.submodule_commit_id)
                  ...
                  (sql/join :branches_commits [:=
                                                    ...
                                                    :branches_commits.commit_id])
                  (sql/where [:in :submodules.commit_id commit-ids])
                  (sql/where [:= :submodules.path path])
                  (sql-format))
        res (->> query
                 (jdbc-sql/query  (get-ds))

...
(defn import-submodule-refs [j-tree-id ^Repository repository]
  (debug 'import-submodule-refs j-tree-id repository)
  ...
  (let [submodule-walk (doto (SubmoduleWalk. repository)
                         (.setTree j-tree-id))]
    (debug 'submodule-walk submodule-walk)
    (debug 'path (.getPath submodule-walk))
    ;(debug 'modules-path (.getModulesPath submodule-walk))
    (debug 'ObjecetId (.getObjectId submodule-walk))
    (debug 'Resolved (.resolve repository (.getName (.getObjectId submodule-walk))))
    )
  ...
  (let [tree-walk (TreeWalk. repository)]
    ...
    (while (.next tree-walk)
      (when (= (.getFileMode tree-walk 0) FileMode/GITLINK)
        (debug "SUBMODULE: " (.getPathString tree-walk) (.getName (.getObjectId tree-walk 0)))

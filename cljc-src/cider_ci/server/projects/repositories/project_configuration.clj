...
(ns cider-ci.server.projects.repositories.project-configuration
  (:refer-clojure :exclude [str keyword])
  (:require [cider-ci.utils.core :refer [keyword str]])

  (:require
    [cider-ci.server.projects.repositories.project-configuration.expansion :as expansion]
    [cider-ci.server.projects.repositories.project-configuration.shared :as shared]
...
(def ^:private CONFIG-FILES-ALTERNATIVES
  ["cider-ci_v5.yml" ".cider-ci_v5.yml"
   "cider-ci_v5.json" ".cider-ci_v5.json"
...
(defn- get-content-or-nil [id path]
  (try (->> (shared/get-content id path [])
            (shared/parse-path-content path))
...
(defn- project-configuration_unmemoized [id]
  (->> (get-either-configfile-content id)
       (expansion/expand id)))

(def project-configuration
  (memo/lru #(project-configuration_unmemoized %)
            :lru/threshold 500))

; to disable caching temporarily:
(def project-configuration project-configuration_unmemoized)
